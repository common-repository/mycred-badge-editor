...
import mycredIcon from './images/mycred-icon.png';
...
const App = () => {
  ...
  const [mbeFormat, setMbeFormat] = useState('')
  const [mbeModalShow, setMbeModalShow] = useState(false)
  const [mbeBtnDisabled, setMbeBtnDisabled] = useState(false)
  ...
  const downloadBadge = () => {
    ...
    setMbeBtnDisabled(true);

    const regex = /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i;

    if( subscriberEmail != '' && regex.test( subscriberEmail ) ) {
      
      let formData = new FormData();
      formData.append('action', 'mbe-subscribe');
      formData.append('email', subscriberEmail);
      formData.append('nonce', window.mbe_data.nonce);

      const requestMetadata = {
        method: 'POST',
        body: formData
      };
      fetch(window.ajaxurl, requestMetadata)
      .then(res => res.json())
      .then(response => {
          if( response.result == 'success' ) {      
            setMbeModalShow(false);
            window.mbe_data.is_mycred_opt_in = 1;
            setSubscriberError('');
            getMbeCanvas();
          }
          else {
            setSubscriberError(response.msg);
          }
          setMbeBtnDisabled(false);
      });

    }
    else {
      setSubscriberError(__('Please enter a valid email address.'));    
      setMbeBtnDisabled(false);
    }
  ...
  const getMbeCanvas = () => {
    ...
    if ( window.mbe_data.is_mycred_opt_in == 1 ) {
      ...
      if( mbeFormat != '' ) {
        ...
        else if( mbeFormat == 'png' ) {
          downloadImage(canvas.toDataURL());
        }
        else if( mbeFormat == 'jpg' ) {
      ...
      setMbeModalShow(true);

    }

  }

  const mbeCSS = { display: mbeModalShow ? 'block' : 'none' }
  ...
  return (
    <div id="app">
      ...
      <div className="mbe-modal" style={mbeCSS}>
          <div className="mbe-modal-content">
            <div className="mbe-modal-header">
              <img src={mycredIcon} alt="myCred Icon" />
              <h4 className="mbe-modal-title">{__('Download your badge!')}</h4>
            </div>
            <div className="mbe-modal-body">
              <p>Your free myCred badge is ready! Continue to opt-in and download.</p>
              <label htmlFor="mbeSubscriberEmail">Email Address<span className="asterisk">*</span></label>
              <br />
              <input 
                id="mbeSubscriberEmail"
                type="email" 
                value={subscriberEmail}
                onChange={e => setSubscriberEmail(e.target.value)}
                onBlur={e => setSubscriberEmail(e.target.value)}
              />
              {subscriberEmail == '' && <div className="mbe-error">{__('This is a required field.')}</div>}
              {(subscriberEmail != '' && subscriberError != '' ) && <div className="mbe-error">{subscriberError}</div>}
            </div>
            <div className="mbe-modal-footer">
              <button className="button mbe-btn-close" type="button" onClick={ () => setMbeModalShow(false) }>Cancel</button>
              <button className="mbe-btn" disabled={mbeBtnDisabled} type="button" onClick={downloadBadge}>Download</button>
            </div>
          </div>
      </div>
    </div>
